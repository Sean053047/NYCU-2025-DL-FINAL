...
class VideoDatasetWithResizingTracking(VideoDataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors:
            try:
                image_latents, video_latents, tracking_map, prompt_embeds = self._preprocess_video(self.video_paths[index], self.tracking_paths[index])
            except ValueError:
    ...
    def _load_preprocessed_latents_and_embeds(self, path: Path, tracking_path: Path) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        images = (
            torch.load(image_latent_filepath, map_location="cpu", weights_only=False) if self.image_to_video else None
        )
        latents = torch.load(video_latent_filepath, map_location="cpu", weights_only=False)
        tracking_map = torch.load(tracking_map_filepath, map_location="cpu", weights_only=False)
        embeds = torch.load(embeds_filepath, map_location="cpu", weights_only=False)
